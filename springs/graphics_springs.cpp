...
void display(void)
{
	...
	//EulerStep(PS, gDeltaT);
	//MidpointStep(PS, gDeltaT);
	RungeKuttaStep(PS, gDeltaT);
	...
	for(i=0; i<NF; i++)
	{
		...
		if(f->Type() == SPRING_FORCE)
		{
			...
			double red = sf->GetRed();
			double green = sf->GetGreen();
			double blue = sf->GetBlue();
			GLdouble color[] = { red, green, blue, 1.0 };
			glColor3dv(color);
...
void InitFinal() {
	...
	while (!fin.eof()) {
		...
		char keyword[20];
		fin >> keyword;
		if (strcmp(keyword, "Particle") == 0) {
			bool anchor;
			double x, y, dx, dy, r;
			fin  >> x >> y >> dx >> dy >> r >> anchor >> ws;
			Particle *p = new Particle(x, y, dx, dy, r, anchor);
			PS.AddParticle(p);

		}
		if (strcmp(keyword, "SpringForce") == 0) {
			int a, b;
			fin >> a >> b >> ws;
			PS.AddForce(new SpringForce(PS.GetParticle(a), PS.GetParticle(b), kSpringConstant, kDampingConstant));
		}
		if (strcmp(keyword, "SpringForceColor") == 0) {
			int x, y;
			double r, g, b;
			fin >> x >> y >> r >> g >> b >> ws;
			PS.AddForce(new SpringForce(PS.GetParticle(x), PS.GetParticle(y), kSpringConstant, kDampingConstant,r,g,b));
		}
		if (strcmp(keyword, "DeltaT") == 0) {
			double d;
			fin >> d >> ws;
			gDeltaT = d;
		}
	...
	Force * DF = new DragForce(.0002, &PS);
	PS.AddForce(DF);
}
